...
for (j = 0; j < rows; j++) {
    ...
    ds_map_add(cell, "f", -1); //f score not calculated
    ds_map_add(cell, "g", -1); //g score not calculated
...
start = ds_grid_get(grid, 0, rows-1);
...
//start[?"g"] = 1;
//start[?"f"] = 1 + point_distance(0, 0, goal[?"x"], goal[?"y"]);
