// Algoritmo Wavefront para búsqueda de ruta v02
...
if (estado == 0) {//Algoritmo BSF
    ...
    for (i=0; i<4; i+=1) {
        ...
        if celda_valida2(n) {
            if (n.dis < nodo.dis and n.dis != -1) {
                nodo = n; //siguiente nodo en la ruta
                path_add_point(ruta, nodo.x*w + w2, nodo.y*h + h2, 0);
                break;
            }
        }
...
#define celda_valida2
/// celda_valida(nodo);
// Una celda es valida si no esta fuera de la matriz, 
// y no esta ocupada por un ostaculo.
